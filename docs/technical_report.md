...
本项目实现一个短剧互动平台原型，面向竖屏短剧播放场景，在常规视频播放基础上叠加弹幕、高光事件、互动点击、打脸小游戏、粒子反馈和互动数据上报。项目采用前后端分离结构，单人交付时以 Web 端 + FastAPI 服务端作为主要验收链路，同时保留 Expo / React Native 移动端原型。

## 2. 模块分析和拆解

| 模块 | 目录 | 职责 |
|---|---|---|
| Web 播放端 | `web/` | 竖屏短剧播放器、弹幕层、打脸互动、粒子效果、进度与操作 UI |
| 移动端原型 | `mobile/` | Expo / React Native 播放器原型，包含互动浮层、连击反馈和移动端组件 |
...
- Web 端可运行竖屏短剧播放器，并在第 5 集触发打脸互动和粒子反馈。
- 移动端已有 Expo / React Native 原型代码，可作为 Android/iOS 预览基础。
...
最终演示建议以 Web 端作为主展示入口，配合 `demo/` 录屏、后端 Swagger 文档和飞书技术文档完成验收。
