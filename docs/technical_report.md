...
本项目实现一个短剧互动平台原型，面向竖屏短剧播放场景，在常规视频播放基础上叠加弹幕、高光事件、互动点击、打脸小游戏、粒子反馈和互动数据上报。项目采用前后端分离结构，最终交付口径为移动端 + FastAPI 服务端，Web 端作为辅助演示和联调入口。

## 2. 模块分析和拆解

| 模块 | 目录 | 职责 |
|---|---|---|
| 移动端 | `mobile/` | Expo / React Native 播放器，包含互动浮层、连击反馈、打脸互动和移动端组件 |
| Web 播放端 | `web/` | 竖屏短剧播放器、弹幕层、打脸互动、粒子效果、进度与操作 UI，作为辅助演示入口 |
...
| Web 前端 | React + Vite + TypeScript | 启动快、构建简单、适合高互动播放器原型 |
| Web 样式 | Tailwind CSS | 快速实现响应式竖屏播放器和状态反馈 |
| 移动端 | Expo + React Native | 支持 Android/iOS 原型预览，组件逻辑可与 Web 端复用思路 |
...
```mermaid
flowchart TD
  A["用户打开 Web/移动端播放器"] --> B["加载本地剧集和视频资源"]
  B --> C["根据播放进度匹配高光事件"]
  C --> D{"是否进入互动片段"}
...
| 第 5 阶段 | Expo 移动端原型、组件迁移、移动端交互 | Android/iOS 预览原型 |
| 第 6 阶段 | 测试、文档、录屏、最终交付整理 | 测试结果、技术文档、演示录屏 |

## 6. 当前完成情况

- 后端提供健康检查、剧集、弹幕、事件流、互动上报、互动汇总、测验、同款搜索和高光检测接口。
- 移动端已有 Expo / React Native 工程，配置了 Android package、iOS bundle identifier、Expo 图标/启动页资源和 EAS 构建配置。
- Web 端可运行竖屏短剧播放器，并在第 5 集触发打脸互动和粒子反馈，可作为辅助演示。
...
最终演示建议以移动端作为主展示入口，配合 `demo/` 录屏、后端 Swagger 文档和飞书技术文档完成验收；Web 端作为浏览器辅助演示入口。
